...
class TestExfiltrationTrench(unittest.TestCase):
    ...
    def test_exfiltration_capacity_rate_independent_of_wq_volume(self):
        # The rate capacity (cfs/gpm) must depend only on geometry/K/head,
        # never on any water-quality/rainfall volume -- it isn't even a
        # parameter to exfiltration_capacity_cfs().
        trench = self._trench()
        cfs = trench.exfiltration_capacity_cfs()
        self.assertGreater(cfs, 0.0)
        self.assertAlmostEqual(trench.exfiltration_capacity_gpm(), cfs * 448.831169, places=4)

    def test_exfiltration_capacity_rate_matches_provided_capacity_backcalc(self):
        # Cross-check against the existing (volume, design_period_hours)
        # method: dividing the 1-hour provided volume by 3600 s must
        # reproduce exfiltration_capacity_cfs() exactly, since both derive
        # from the same denominator/rate internally.
        trench = self._trench(percent_wq_required=1.0)
        rate_cfs = trench.exfiltration_capacity_cfs()
        vol_1hr_cuft = trench.provided_capacity_cuft(design_period_hours=1.0)
        self.assertAlmostEqual(vol_1hr_cuft / 3600.0, rate_cfs, places=8)
