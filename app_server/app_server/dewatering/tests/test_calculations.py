...
from dewatering.calculations import (
    ...
    PermitThresholds, screen_permit_thresholds,
)
...
def test_tank_freeboard_reduces_effective_volume_only():
    base = design_settling_tank(TankInputs(flow_gpm=227.5566, length_ft=7, width_ft=4, depth_ft=6))
    with_freeboard = design_settling_tank(TankInputs(
        flow_gpm=227.5566, length_ft=7, width_ft=4, depth_ft=6, freeboard_and_sediment_fraction=0.2))
    # Gross numbers (volume, Vm, Vs, SOR) must be unaffected by freeboard.
    assert with_freeboard.volume_cf == base.volume_cf
    assert with_freeboard.mean_horizontal_velocity_ft_s == base.mean_horizontal_velocity_ft_s
    # Effective volume/detention must reflect the 20% reservation.
    assert abs(with_freeboard.effective_volume_cf - base.volume_cf * 0.8) < 1e-9
    assert with_freeboard.effective_detention_time_min < base.detention_time_min
    assert any("freeboard" in w.lower() for w in with_freeboard.warnings)


def test_selected_tank_system_pass_fail_and_ratio():
    passing = check_selected_tank_system(
        SelectedTankSystem(name="25 CY Sediment Tank", provided_volume_gal=25 * 27 * 7.480519480519481),
        flow_gpm=227.5566, required_volume_gal=1256.73,
    )
    assert passing.status == "PASS"
    assert passing.capacity_ratio > 1.0

    failing = check_selected_tank_system(
        SelectedTankSystem(name="Undersized Tank", provided_volume_gal=500.0),
        flow_gpm=227.5566, required_volume_gal=1256.73,
    )
    assert failing.status == "FAIL"
    assert failing.capacity_ratio < 1.0


def test_permit_screening_returns_none_when_unconfigured():
    summary = summarize_water_balance([])
    result = screen_permit_thresholds(summary, PermitThresholds())
    assert result is None


def test_permit_screening_returns_rows_when_configured():
    from dewatering.calculations import WaterBalanceSummary
    summary = WaterBalanceSummary(average_flow_gpm=100, max_flow_gpm=120,
                                   max_daily_pumpage_mgd=0.1, total_project_pumpage_mg=1.0,
                                   total_pumping_days=5)
    result = screen_permit_thresholds(summary, PermitThresholds(max_average_gpm=50))
    assert result is not None
    assert len(result) == 1
    assert result[0].status == "EXCEEDS"
