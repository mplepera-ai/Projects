    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(suffix=".pdf", delete=False) as tmp:
            export_dewatering_calc_pdf(
                ...
                permit_screening=core["permit_screening"],
                tank_inputs=core["tank_inputs"],
                tank_result=core["tank_result"],
                min_tank_result=core["min_result"],
                tank_mode=core["tank_mode"],
                selected_check=core["selected_check"],
                receiving_checks=core["receiving_checks"],
                project_info=data.get("projectInfo"),
                criteria_basis=data.get("criteriaBasis"),
                extra_assumptions=data.get("extraAssumptions"),
                regulatory_basis=data.get("regulatoryBasis"),
            )
            tmp_path = tmp.name
